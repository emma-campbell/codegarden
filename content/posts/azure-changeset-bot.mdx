...
status: "published"
tags:
  - { title: "@changesets/cli", slug: "changesets" }
  - { title: "Azure DevOps", slug: "azure-devops" }
  - { title: "Azure Functions", slug: "azure-functions" }
  - { title: "Developer Tooling", slug: "developer-tooling" }
---
...
1. We wanted human readable descriptions of the changes made to each project, but we didn't want to add to much extra effort into commiting these changes.
...
We had one issue with moving forward with this route -- there was no readibly available integrations with Azure DevOps. 
...
Finally, using the command `yarn changeset publish` will publish the changes to to the registry.
...
To brainstorm, we decided to map out the ideal steps to our pipeline steps.
...
Changeset also allows an empty changeset for changes that aren't meant to be "released", per say. For example of you have commits adding developer tooling, changes to a `tsconfig`, etc.

## Bot Steps

To accomplish the above, we need our bot to check for for two things.
...
2. If the changeset requires a version bump (the changeset is _not_ empty), run `changeset version`, and push those changes to the PR.
...
I'll run over the instructions for getting started. I am using MacOS, but the only step this affects is using the homebrew package manager to install Azure Functions Core Tools. For other operating systems, check out [Azure's documentation](https://learn.microsoft.com/en-us/azure/azure-functions/functions-run-local?tabs=v4%2Cmacos%2Ccsharp%2Cportal%2Cbash#install-the-azure-functions-core-tools).
...
Using homebrew, we will start by install the core tools.
...
Create a new directory to store your bot related functions in.
...
At my job, we use yarn instead of NPM. These next few steps are option, but if you'd like to do the same changing these will incorporate yarn as the package manager.
...
this command with run `tsc` in watch mode, so it will rebuild automatically when you make changes.
...
Once you have your token, create a `.env` file in the root directory of the project will the following

```sh showLineNumbers title=".env"
AZURE_PAT="<YOUR_TOKEN>"
```

replacing `<YOUR_TOKEN>` with the token you generated. We will start off by making a utility directory to hold some functions that make working with the API code a little bit easier.
...
We will start by defining a method that will our environment variables for us.
...
Let's use a class to represent all the bot operations we wish to preform. Create a new file in the function directory, called `changebot.ts` and create a new class. Our constructor for this class will take the parameters `api`, to interact with Azure's GitApi and `pr` to have the relevant pull request information.
...
We can this use this to get a list of all changed files and search it for changesets.
